...
class ConditionalMoEReconstructor(nn.Module):
    ...
    def forward(self, query: torch.Tensor, retrieved_missing: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        expert_outputs = []
        expanded_query = query.unsqueeze(1).expand(-1, actual_k, -1)
        expert_inputs = torch.cat([expanded_query, retrieved_missing], dim=-1)
        for expert_index in range(actual_k):
            expert_outputs.append(self.experts[expert_index](expert_inputs[:, expert_index]))
...
class SampleAwarePromptGenerator(nn.Module):
    ...
    def forward(
        ...
        prompt_input = torch.cat([query, attended.squeeze(1), weighted_context], dim=-1)
